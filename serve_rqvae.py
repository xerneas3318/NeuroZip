"""
serve_rqvae.py — port-8011 comparison viewer: RQ-VAE vs fidelity benchmark.

For each held-out concept: stimulus image, the trial-averaged ACTUAL EEG, and
both codecs' reconstructions side by side, with the PER-IMAGE MSE for each model,
plus per-model stats (bpp, bytes/epoch, compression ratio). UI styled after
rian's demo_clean.html. Self-contained (no open_clip / judge).

Run:
    ./serve_rqvae.sh     # or: python serve_rqvae.py --port 8011
"""
...
SRC_BITS = N_CHANNELS * N_TIMES * 16     # float16 source bits per epoch
...
header{display:flex;align-items:baseline;gap:14px;padding:26px 0 4px}
h1{margin:0;font-size:1.7em;letter-spacing:-.5px}h1 span{color:var(--accent)}.sub{color:var(--ink-2);margin:2px 0 0}
.stats{display:flex;gap:12px;flex-wrap:wrap;margin:16px 0}
.cell{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:11px 16px;box-shadow:var(--shadow);min-width:110px}
.cell .v{font-size:1.35em;font-weight:700}.cell .v.us{color:var(--accent)}.cell .v.them{color:var(--neutral)}
.cell .k{color:var(--ink-muted);font-size:.74em;text-transform:uppercase;letter-spacing:.5px}
.controls{display:flex;gap:12px;align-items:center;flex-wrap:wrap;margin:6px 0 14px}
...
.stim img{width:100%;border-radius:10px;display:block}.stim .cap{color:var(--ink-muted);font-size:.82em;margin-top:6px;text-align:center}
...
table{width:100%;border-collapse:collapse;font-size:.92em}th,td{text-align:right;padding:7px 10px;border-bottom:1px solid var(--border)}
th:first-child,td:first-child{text-align:left}thead th{color:var(--ink-muted);font-weight:600;font-size:.82em;text-transform:uppercase;letter-spacing:.4px}
...
def load_any(ckpt, device):
    ...
    fv = ck["final_val"]
    bits = fv.get("ratio") and SRC_BITS / fv["ratio"]
    return {"model": m, "kind": kind, "cfg": c, "fv": fv,
            "ratio": fv["ratio"], "bits": bits, "bytes": bits / 8, "mse_all": fv["mse"]}
...
def index():
    ...
    for name, e in S["models"].items():
        cls = "us" if e["kind"] == "rqvae" else "them"
        cells += (f'<div class=cell><div class="v {cls}">{e["mse_all"]:.4f}</div>'
                  f'<div class=k>{name} · MSE (all)</div></div>')
    cells += (f'<div class=cell><div class=v>{S["ratio"]:.0f}×</div><div class=k>compression</div></div>'
              f'<div class=cell><div class=v>{S["bytes"]:.0f} B</div><div class=k>bytes / epoch</div></div>')
    return f"""<!doctype html><html><head><meta charset=utf-8><title>NeuroZip · RQ-VAE vs fidelity</title>
<style>{CSS}</style></head><body><div class=wrap>
<header><h1>Neuro<span>Zip</span></h1><div class=sub>RQ-VAE vs fidelity benchmark · held-out, trial-averaged test EEG · matched compression</div></header>
<div class=stats>{cells}</div>
<div class=controls><label>concept&nbsp;<select id=sel onchange=upd()>{opts}</select></label>
<button onclick=rnd()>Random</button></div>
<div class=row>
 <div class=stim><img id=stim><div class=cap id=stimcap>stimulus</div>
   <table id=info><thead><tr><th>model</th><th>MSE</th><th>bpp</th></tr></thead><tbody></tbody></table>
 </div>
 <div class=card><h3 id=heat-h>actual vs reconstructions (normalized σ)</h3><img id=heat></div>
</div>
<div class=card><h3>RQ-VAE discrete latent codes · per-channel waveforms (actual vs both models)</h3><img id=detail></div>
<p class=note>Per-image MSE is computed live for this epoch. bpp = bits per EEG sample;
bytes/epoch = compressed size. RQ-VAE stores {S['models'].get('RQ-VAE',{}).get('cfg',{}).get('num_quantizers','?')}×32 discrete codes.</p>
<script>
async function info(i){{const r=await fetch('/api/info?idx='+i);const d=await r.json();
let best=Math.min(...Object.values(d.models).map(m=>m.mse));
let rows=Object.entries(d.models).map(([n,m])=>`<tr><td class="model-${{m.kind=='rqvae'?'us':'them'}}">${{n}}</td>`+
 `<td class="${{m.mse==best?'win':''}}">${{m.mse.toFixed(4)}}</td><td>${{m.bpp.toFixed(3)}}</td></tr>`).join('');
document.querySelector('#info tbody').innerHTML=rows;
document.getElementById('heat-h').textContent='actual · '+Object.keys(d.models).join(' · ')+'  (per-image MSE shown on each)';}}
...
@app.route("/api/info")
def api_info():
    i = int(request.args.get("idx", 0)); x = S["te"][i:i+1]
    out = {"concept": S["concepts"][i], "models": {}}
    for name, e in S["models"].items():
        xh = recon(e, x)
...
def heat():
    ...
    for name, e in S["models"].items():
        ...
        panels.append((f"{name}  MSE={mse:.4f}", xr))
    vmax = float(np.max([np.abs(p[1]).max() for p in panels]))
    n = len(panels)
    fig, ax = plt.subplots(1, n, figsize=(4.0 * n, 3.4))
    ext = [-200, 996, 63, 0]
    for a, (t, arr) in zip(ax, panels):
...
def detail():
    i = int(request.args.get("idx", 0)); x = S["te"][i:i+1]
    xo = x[0].cpu().numpy()
    rq = S["models"].get("RQ-VAE")
    fig = plt.figure(figsize=(12, 3.6))
    gs = fig.add_gridspec(1, 3, width_ratios=[1.0, 1.6, 1.6], wspace=0.32)
    axc = fig.add_subplot(gs[0])
    if rq is not None:
        with torch.no_grad():
            z = rq["model"].encoder(x).transpose(1, 2)
            _, codes, _ = rq["model"].rvq(z)
        ...
        axc.axis("off")
    recons = {name: recon(e, x)[0].cpu().numpy() for name, e in S["models"].items()}
    t = np.linspace(-200, 996, N_TIMES)
    colors = {"rqvae": "#d62728", "scalar": "#6b7388"}
    for k, ch in enumerate([18, 47]):
        ax = fig.add_subplot(gs[k + 1])
        ax.plot(t, xo[ch], color="0.3", lw=1.3, label="actual")
        for name, e in S["models"].items():
            ax.plot(t, recons[name][ch], color=colors[e["kind"]], lw=1.0,
...
def main():
    ...
    S.update(model=head, models=models, te=avg.to(device), concepts=texts, img_paths=imgs,
             ratio=head["ratio"], bytes=head["bytes"])
    print(f"serving on http://{args.host}:{args.port}/  models={list(models)}")
